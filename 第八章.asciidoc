...
address.transfer(amount):: 将金额（wei）转移到该地址，并在发生任何错误时抛出异常。我们在+Faucet+示例中的+msg.sender+地址上使用了此函数，+payable(msg.sender).transfer()+。
...
构造函数是名为+constructor+的特殊函数，它在合约创建期间运行一次，用于初始化合约状态。
...
contract caller {
	...
	function make_calls(calledContract _calledContract) public {
		...
		require(success);
		(success, ) = address(_calledContract).delegatecall(abi.encodeWithSignature("calledFunction()"));
		require(success);
	}
}
----
