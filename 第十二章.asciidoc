...
interface Oracle {
    function initRequest(uint256 queryType, function(uint256) external onSuccess, function(uint256) external onFailure) external returns (uint256 id);
    function addArgumentToRequestUint(uint256 id, bytes32 name, uint256 arg) external;
    function addArgumentToRequestString(uint256 id, bytes32 name, bytes32 arg) external;
    function executeRequest(uint256 id) external;
    function getResponseUint(uint256 id, bytes32 name) external view returns(uint256);
    function getResponseString(uint256 id, bytes32 name) external view returns(bytes32);
    function getResponseError(uint256 id) external view returns(bytes32);
    function deleteResponse(uint256 id) external;
}
...
contract OracleB1IQClient {
    ...
    constructor(address addr) payable {
        oracle = Oracle(addr);
        getIntraday("IBM", block.timestamp);
    }
